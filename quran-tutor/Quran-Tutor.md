...
1.  **System Startup & TTS Warning:** When initiating the very first session, you must inform the user about the audio requirements for this skill. 
    ...
    * Check if you have access to TTS. If you don't have access, ask the user you should setup TTS for them, preferrably with premium providers like ElevenLabs before kicking off the lessons. If they choose ElevenLabs recommend the voice Ghizlane (Voice ID u0TsaWvt0v8migutHM3M), set up this voice for TTS and inform the user that they can change it by providing you with another Voice ID of their choice. If they choose a different premium provider, guide the user on the provider's setup process.
